...
use crate::ffi::lora::{validate_lora_header, LoRAState};
use crate::ffi::types::{ContextParams, LoadParams};
...
impl LlamaModel {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P, params: LoadParams) -> Result<Self, FfiError> {
        ...
        let ptr = unsafe {
            llama_cpp_rs::llama_load_model_from_file(path_str, params.into())
                .map_err(|e| FfiError::Internal(format!("模型加载失败: {}", e)))?
        };

        let ptr = NonNull::new(ptr).ok_or(FfiError::Internal(
            "llama_load_model_from_file返回空指针".to_string(),
        ))?;
    ...
    where
        F: FnOnce(*const llama_cpp_rs::llama_model) -> R,
    {
        let model = self
            .inner
            .lock()
    ...
    pub fn apply_lora<P: AsRef<Path>>(&self, lora_path: P) -> Result<(), FfiError> {
        ...
        let path_str = path
            .to_str()
            .ok_or_else(|| FfiError::InvalidParameter("路径非法".into()))?;
        ...
        // 应用新LoRA
        let result = unsafe {
            llama_cpp_rs::llama_model_apply_lora_from_file(
                model.ptr.as_ptr(),
                path_str,
                1.0,
                std::ptr::null_mut(),
    ...
    fn unload_lora_inner(&self, model: &mut InnerModel) -> Result<(), FfiError> {
        unsafe {
            llama_cpp_rs::llama_model_remove_lora(model.ptr.as_ptr())
                .map_err(|e| FfiError::Internal(format!("卸载失败: {}", e)))
        }
    }
}
...
impl LlamaContext {
    pub fn new(model: &LlamaModel, params: ContextParams) -> Result<Self, FfiError> {
        if !is_backend_initialized() {
            initialize_backend()?;
        }
        ...
        // - backend已初始化
        let model_inner = model
            .inner
            .lock()
            .map_err(|_| FfiError::Internal("锁中毒".into()))?;
        let ctx_ptr = unsafe {
            llama_cpp_rs::llama_new_context_with_model(model_inner.ptr.as_ptr(), params.into())
        };
            ...
            Ok(ptr) => {
                if ptr.is_null() {
                    return Err(FfiError::Internal(
                        "llama_new_context_with_model 返回空指针".to_string(),
                    ));
                }
                ptr
            }
        ...
        Ok(Self {
            ...
            ctx_ptr: NonNull::new(ctx_ptr).ok_or(FfiError::Internal("返回空指针".to_string()))?,
            _marker: PhantomData,
        })
    }
}
