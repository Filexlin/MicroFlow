...
use crate::ffi::{FfiError, LoadParams};
#[cfg(feature = "llama")]
...
impl VramPool {
    ...
    pub fn load_lora(&mut self, model_id: &str, lora_path: &Path) -> Result<(), FfiError> {
        let slot = self
            .slots
            .get_mut(model_id)
            .ok_or_else(|| FfiError::ModelNotFound(PathBuf::from(model_id)))?;
    ...
    fn available_vram(&self) -> Result<usize, FfiError> {
        ...
        let used: usize = self
            .slots
            .values()
            .map(|s| s.model.size_bytes() + s.current_lora_size)
            .sum::<Result<usize, FfiError>>()?;
...
impl VramPool {
    ...
    pub fn load_model(
        ...
        Err(FfiError::Internal("Llama feature not enabled".into()))
    }

    /// 获取模型
    pub fn get_model(&mut self, _id: &str) -> Option<Arc<()>> {
        None
    }
    ...
    pub fn load_lora(&mut self, _model_id: &str, _lora_path: &Path) -> Result<(), FfiError> {
        Err(FfiError::Internal("Llama feature not enabled".into()))
    }

    pub fn unload_lora(&mut self, _model_id: &str) -> Result<(), FfiError> {
        Err(FfiError::Internal("Llama feature not enabled".into()))
    }

    pub fn switch_lora(&mut self, _model_id: &str, _lora_path: PathBuf) -> Result<(), FfiError> {
        Err(FfiError::Internal("Llama feature not enabled".into()))
    }
}
